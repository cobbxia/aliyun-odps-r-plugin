...
.init_odps_operator <- function(conf){
     if(is.na(conf['log4j_properties'])){
         odpsOperator <<- .jnew("com/aliyun/odps/rodps/ROdps", conf["project_name"],conf["access_id"],conf["access_key"] , conf["end_point"], conf["dt_end_point"],"","")
     }
     else{
         odpsOperator <<- .jnew("com/aliyun/odps/rodps/ROdps", conf["project_name"],conf["access_id"],conf["access_key"] , conf["end_point"], conf["dt_end_point"],"",.jnew("java/lang/String",conf["log4j_properties"]))}
     rodps.init.type()
}
