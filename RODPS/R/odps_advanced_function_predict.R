...
rodps.predict.fda <- function(object, srctbl, tgttbl, type='class', prior, dimension=2)
{
  ...
  dist <- function(x, mean, m = ncol(mean)) (scale(x, mean, FALSE)^2) %*% rep(1, m)　#这个转为ＳＱＬ，计算距离
  ...
  #y <- predict(object$fit, newdata)　　#假设object$fit$degree=1, object$fit$monomial=FALSE
  yvar.name=paste(all.vars(object$terms)[attr(object$terms,'response')],seq(1:ncol(object$fit$coefficients)), sep="_ln")
  ...
  #根据alpha值缩放，合并到上一步，sql.tran中
  #newdata <- scale(y, FALSE, sqima * alpha)
  sa = sqima * alpha
  #投影+缩放
  theta = object$theta[, seq(dimension), drop = FALSE]
  ytran.name =  paste(all.vars(object$terms)[attr(object$terms,'response')],seq(1:ncol(object$fit$coefficients)), sep="_tr")
  var.exp = c()
  for(i in seq(1:ncol( theta))){
    var.exp[i] = sprintf( " (%s)/%f ", paste( theta[,i], yvar.name , sep='*', collapse="+"), sa[i])
  }
  sql.tran.exp = paste( var.exp, ytran.name, sep=" AS " , collapse="  ,\n  ")
  sql.tran.exp = gsub("\\+-", "-", sql.tran.exp)
  sql.tran = sprintf(" SELECT *, \n %s \n FROM (\n %s ) sub1 \n", sql.tran.exp, sql.linear)
  if (missing(prior))
    prior <- object$prior
  else {
  ...
  #计算距离，需要一个类似ＷＫＮＮ，根据距离映射到类别的通用函数（ＰＹＴＨＯＮ）
  #值与means中心点的欧氏距离-prior,
  #least_index ，输入一组值，返回最小值所在位置，距离值为各维度差值的平方和
  prior <- 2 * log(prior)
  dist_list  = c()
  ...
  dist_exp = gsub("\\+-", "-", dist_exp)
  label_exp = paste(" WHEN ", seq(nrow(means)), " THEN '", rownames(means), "'", sep="", collapse="")
  case_exp = sprintf(" CASE least_index(%s) \n %s \n  END AS %s\n", dist_exp, label_exp, "predict_v")
  sql = sprintf(" CREATE TABLE %s AS \nSELECT *, \n%s \n FROM (\n %s \n) sub_2", tgttbl, case_exp, sql.tran)
}
