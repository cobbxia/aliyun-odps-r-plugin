...
public class RDTDownloader extends DTProcess<DownloadWorker,DownloadSession> {
	...
	static String ptvs="ptvs";
	public RDTDownloader(Context<DownloadSession> context) throws IOException {
		super(context);
	}
	
	public List<List<String>> downloadTable(String tempDataFile)
			throws ROdpsException, IOException {
    ...
    try {
      if (context.getPartition() != null && !context.getPartition().isEmpty()){
        ...
        downloadSession = tunnel.createDownloadSession(context.getProject(),
            context.getTable(), partitionSpec);
      } else{
        downloadSession = tunnel.createDownloadSession(context.getProject(),
            context.getTable());
      }
      ...
      throw new ROdpsException(e.getErrorCode() + e.getErrorMsg());
		...
		List<List<String>> ret = genTableSchema(ptkv == null ? null
				: (List<String>) ptkv.get(0));
		try {
			...
			throw new IOException(
					"down load table met InterruptedException", e);
		}
		return ret;
	}
	
	@Override
	public DownloadWorker createWorker(int threadId, Context<DownloadSession> context, long startRecordNumber,
	...
	* @throws
	...
	private List<List<String>> genTableSchema(List<String> keys) {
		...
		for (int i = 0; i < columnNumber; ++i) {
			...
			ret.get(1).add(this.context.getSchema().getColumn(i).getType().toString().replace("ODPS_", "").toLowerCase());			
		}
		if(keys!=null){
			for(String k:keys){
				ret.get(0).add(k);
	...
	public List<Object> genPartitionCell() throws ROdpsException {
		if (this.context.getPartition() == null
				|| this.context.getPartition().isEmpty()) {
			return null;
		}
