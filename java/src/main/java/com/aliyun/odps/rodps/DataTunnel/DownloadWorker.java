	...
	public void run() {
		try {
			...
			if (downloadRecordNumber > 0) {
				reader = context.getAction().openRecordReader(
						startRecordNumber, downloadRecordNumber);
			}
			midStorage.saveDtData(reader, downloadRecordNumber);

			LOG.info("threadId=" + this.threadId
					+ " download finished,loadedRecordNum="
